...
public class PercentageUpgradeRegTest extends BaseTestModel {
    ...
    private void stage1() throws IOException, AirlockInvalidFileException, AirlockNotInitializedException, InterruptedException {
        testHelper.recreateClient("7.8");
        AirlockProductManager manager = testHelper.getManager();
        //Add relevant user group
        manager.getUserGroupsService().setDeviceUserGroups(ug);
        manager.getProductInfoService().setAllowExperimentEvaluation(false);
        //1.pull-calc-sync
        testHelper.pull();
        ...
        Assert.assertTrue(manager.getFeaturesService().getFeature("ns1.R1 L1 100").getSource().equals(Feature.Source.SERVER) && manager.getFeaturesService().getFeature("ns1.R1 L1 100").isOn());
        //These features are defined to be 0% and should be OFF
        Assert.assertTrue(manager.getFeaturesService().getFeature("ns1.R1 0").getSource().equals(Feature.Source.SERVER) && !manager.getFeaturesService().getFeature("ns1.R1 0").isOn());
        Assert.assertTrue(manager.getFeaturesService().getFeature("ns1.R2 0").getSource().equals(Feature.Source.SERVER) && !manager.getFeaturesService().getFeature("ns1.R2 0").isOn());
    ...
    public void oldToNewSeasonKeepStatusTest() throws InterruptedException, AirlockInvalidFileException, AirlockNotInitializedException, IOException {
        ...
        //Now upgrade
        //Add relevant user group
        testHelper.setup("7.10", ug, null, null, false, true, false);
        //pull-calc-sync
        testHelper.pull();
        testHelper.calcSync(null, null);
    ...
    public void oldToNewSeasonDecreasePercentageTest() throws Exception {
        ...
        //Now upgrade
        //Add relevant user group
        testHelper.setup("7.10", ug, null, null, false, false, false);//Now simulate a server change: increase percentage
    ...
    public void keepRandomsMapTest() throws InterruptedException, AirlockInvalidFileException, AirlockNotInitializedException, IOException {
         ...
         */
        stage1();
        PersistenceHandler ph = testHelper.getManager().getInfraAirlockService().getPersistenceHandler();
        ...
        //3. Upgrade
        testHelper.setup("7.10", ug, null, null, false, false, false);
        testHelper.pull();
    ...
    public void notZerosRandomsTest() throws InterruptedException, AirlockInvalidFileException, AirlockNotInitializedException, IOException {
         ...
         */
        stage1();
        String randomsMap = testHelper.getManager().getInfraAirlockService().getPersistenceHandler().getFeaturesRandomMap().toString();
