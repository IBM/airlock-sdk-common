...
public class FeaturePercentageRealTest extends BaseTestModel {
    ...
    private void test(int index) throws IOException, AirlockInvalidFileException, AirlockNotInitializedException, InterruptedException {
        ...
        Assert.assertTrue("Iteration #" + index + ": Feature source is not server", manager.getFeaturesService().getFeature(FEATURE_NAME).getSource().equals(Feature.Source.SERVER));
        results[index] = manager.getFeaturesService().getFeature(FEATURE_NAME).isOn();
    }
