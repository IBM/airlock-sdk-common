...
public class InMemoryCacheTest extends BaseTestModel {
    ...
    public void inMemoryCachedFeaturesMapTest() throws IOException, AirlockInvalidFileException, JSONException, AirlockNotInitializedException, InterruptedException {
        ...
        Assert.assertNotNull(testHelper.getManager().getInfraAirlockService().getCachedSyncFeatureList().get(InfraAirlockService.SYNC_FEATURE_MAP));
        Assert.assertNotNull(testHelper.getManager().getInfraAirlockService().getCachedPreSyncFeatureList().get(InfraAirlockService.PRE_SYNC_FEATURE_MAP));

        Assert.assertEquals(true, testHelper.getManager().getFeaturesService().getFeature("ns.F1").isOn());
        Assert.assertEquals(false, testHelper.getManager().getFeaturesService().getFeature("ns.F11").isOn());
        Thread.sleep(30000);
        Assert.assertNull(testHelper.getManager().getInfraAirlockService().getCachedSyncFeatureList().get(InfraAirlockService.SYNC_FEATURE_MAP));
        Assert.assertNull(testHelper.getManager().getInfraAirlockService().getCachedPreSyncFeatureList().get(InfraAirlockService.PRE_SYNC_FEATURE_MAP));
