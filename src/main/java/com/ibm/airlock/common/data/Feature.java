...
public class Feature implements Serializable {
    ...
    private final String TAG = "Airlock.Feature";
    ...
    protected List<Feature> children = new CopyOnWriteArrayList<>();
     ...
     **/
    public Feature(JSONObject info) {
    ...
    public String printableToString() {
        ...
        sb.append(Constants.JSON_FEATURE_FULL_NAME + " = " + name + '\n');
        sb.append(Constants.JSON_FEATURE_IS_ON + " = " + isOn + '\n');
        sb.append(Constants.JSON_FEATURE_SOURCE + " = " + source + '\n');
        sb.append(Constants.JSON_FEATURE_CONFIGURATION + " = " + configuration + '\n');
        sb.append(Constants.JSON_FEATURE_CONFIGURATES_STATUSES + " = " + configurationStatuses + '\n');
        sb.append(Constants.JSON_FEATURE_FIELD_PERCENTAGE + " = " + percentage + '\n');
        sb.append(Constants.JSON_ORDERED_WEIGTH + " = " + weight + '\n');
        sb.append(Constants.JSON_FIELD_BRANCH_STATUS + " = " + branchStatus + '\n');
        sb.append(Constants.JSON_FIELD_PURCHASED + " = " + isPurchased + '\n');
        sb.append(Constants.JSON_FIELD_IS_PREMIUM + " = " + isPremium + '\n');
        sb.append(Constants.JSON_FIELD_STORE_PRODUCT_ID + " = " + storeProductId + '\n');


        if (parent != null) {
            sb.append(Constants.JSON_FEATURE_PARENT_NAME + " = " + parent.getName() + '\n');
        }
        if (children != null && children.size() > 0) {
        ...
        sb.append(traceInfo.length() > 0 ? Constants.JSON_FEATURE_TRACE + " = " + traceInfo + '\n' : "");
        return sb.toString();
    }
    ...
    public Map<String, Object> getConfigurationJsonObjectsMap() throws JSONException {
        ...
        for (int i = 0; i < enabledConfigLength; i++) {
            ...
            Object value = BaseRawFeaturesJsonParser.getFieldValueFromJsonObject(configObject, key.split("\\."), true);
            if (value != null) {
                returnedObjects.put(key, value.toString());
            }
        }
        return returnedObjects;
    }
