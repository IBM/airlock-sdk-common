...
public class FeaturesList<T extends Feature> {
    ...
    private static final String TAG = "Airlock.FeaturesList";
    Map<String, T> features;
    private List<String> latestSyncChangedFeatures;
    ...
    public synchronized void put(@Nullable String key, @Nullable T feature) {
        if (key == null || key.trim().isEmpty() || feature == null) {
            return;
        }
    ...
    private synchronized void merge(FeaturesList newValues, boolean addCachedFeatures,  boolean storeChangesList) {
        ...
        if (!oldFeaturesMap.isEmpty() && addCachedFeatures) {
    ...
    public synchronized JSONObject toJsonObject() {
        ...
        try {
            ...
            if (analyticsAppliedRules != null) {
                result.put(Constants.JSON_FEATURE_CONFIG_ANALYTICS_APPLIED_RULES, new JSONArray(analyticsAppliedRules));
            }
