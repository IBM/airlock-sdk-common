...
public class StreamsDevTest extends BaseTestModel {
    ...
    public void featuresUsageBasicTest() throws JSONException, ScriptInitException {
        ...
        eventJsonString = "{\n" +
                ...
                "}";
        events.put(new JSONObject(eventJsonString));
        testHelper.getManager().getStreamsManager().calculateAndSaveStreams(events, true);
        ...
        Assert.assertNull(testHelper.getManager().getStreamsManager().getStreamByName("videoPlayedCounter").getCacheForTest());
        Assert.assertNull(testHelper.getManager().getStreamsManager().getStreamByName("videoPlayedCounter").getResultForTest());
        String featureUsageSummary = testHelper.getManager().getStreamsSummary();
        JSONObject featureUsageSummaryJson = new JSONObject(featureUsageSummary);
        testHelper.getManager().getStreamsManager().persist();
