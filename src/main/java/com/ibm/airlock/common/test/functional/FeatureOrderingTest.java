...
public class FeatureOrderingTest extends BaseTestModel {
    ...
    public void testCalculatedOrder() throws JSONException {
        ...
        List<Feature> rootFeatures = manager.getRootFeatures();
        if (rootFeatures.isEmpty()) {
            return;
        }
        Assert.assertTrue(manager.getFeature("qa.P1").isOn());

        List<Feature> children = manager.getFeature("qa.P1").getChildren();
        JSONArray appliedOrderRules = manager.getFeature("qa.P1").getAnalyticsAppliedOrderRules();
        JSONArray orderedFeatures = manager.getFeature("qa.P1").getAnalyticsOrderedFeatures();
